...
export default function ResetPassword({ route }) {
  ...
  const { email, phone } = route.params; // Nhận cả email và phone từ route params
  console.log("email", email);
  console.log("phone", phone);

  // Kiểm tra nhận email hoặc phone
  const isEmail = !!email; // Nếu có email thì true
  const isPhone = !!phone; // Nếu có phone thì true
  ...
  const handleResetPassword = async () => {
    ...
    try {
      // Gọi API phù hợp dựa trên email hoặc phone
      const response = await api({
        method: typeHTTP.POST,
        url: isEmail ? "/user/reset-password" : "/user/reset-password-phone", // Chọn endpoint phù hợp
        body: isEmail ? { email, newPassword, confirmPassword } : { phone, newPassword, confirmPassword },
        sendToken: false,
      });
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
        <View style={{ flex: 1, justifyContent: "center", alignItems: "center", backgroundColor: "#FFFFFF", paddingHorizontal: 20 }}>
          ...
          <View style={{ alignItems: "center", marginBottom: height * 0.05 }}>
            <Image source={require("../../img/LOGOBLACK.png")} style={{ width: width * 0.35, height: undefined, aspectRatio: 1, resizeMode: "contain", marginBottom: height * 0.02 }} />
          ...
          <View style={{ width: "100%", marginBottom: height * 0.03 }}>
            ...
            <View style={{ position: "relative" }}>
              ...
              />
              <TouchableOpacity style={{ position: "absolute", right: 15, top: 15 }} onPress={() => setConfirmPasswordVisible(!isConfirmPasswordVisible)}>
          ...
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            ...
            <TouchableOpacity>
              <Text onPress={() => navigation.navigate("SignUp")} style={{ color: "#E53935", fontSize: 14, fontWeight: "bold" }}>
                Đăng ký
              </Text>
            </TouchableOpacity>
          </View>
